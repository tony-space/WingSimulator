...
static __global__ void TransformBoxes(const SBoundingBoxSOA boxes, float4* __restrict__ out)
{
...
void CMortonTree::Build(const SBoundingBoxSOA& leafs)
{
	EvaluateSceneBox(leafs);
}

void CMortonTree::EvaluateSceneBox(const SBoundingBoxSOA& leafs)
{
	...
	TransformBoxes <<<gridDim, blockDim >>> (leafs, boxesPtr);
	CudaCheckError();
